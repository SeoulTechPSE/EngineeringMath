...
subtitle: '<font color="red">Chapter 5. Series Solutions of Linear Differential Equations</font>'
format:
  revealjs: 
    slide-level: 3
    slide-number: true
...
-  Substituting $~y=\sum_{n=0}^\infty c_n x^n$ and the second derivative $y''=\sum_{n=2}^\infty c_n n(n -1) x^{n -2}$
   into the DE gives
...
$${\scriptsize
\begin{align*}
   &\;\big\Downarrow \;{\scriptsize\text{It is standard practice to choose}}\; \\
   c_0&=\frac{1}{2^\nu \Gamma(1 +\nu)} \\ 
   &\text{thus, } \\
  c_{2n} &= \frac{(-1)^n}{2^{2n +\nu} n!\, \Gamma(1 +\nu +n)}
  \end{align*}}$$
...
$${\small
     \begin{align*}
       {\scriptsize y'' +\frac{1 -2a}{x} y' }&{\scriptsize +\left( b^2 c^2 x^{2c -2} +\frac{a^2 -p^2 c^2}{x^2}\right)y=0, \;p \geq 0 } \\ 
...
$${\small
     \begin{align*}
       ...
       {\scriptsize s^2 \frac{d^2 x}{ds^2} }&{\scriptsize +s\frac{dx}{ds} +s^2 x = 0 }
  \end{align*}}$$
...
#ax1.set_title('$J_m(x), m=0,1,2,3,4$')
# ax1.set_xlabel('x')
...
$$J_{1/2}(x)={\tiny \sum_{n=0}^\infty \frac{(-1)^n}{n!\, \frac{(2n +1)!}{2^{2n +1} n!} \sqrt{\pi}} \left( \frac{x}{2} \right)^{2n +1/2} = \sqrt{\frac{2}{\pi x}}\sum_{n=0}^\infty \frac{(-1)^n}{(2n +1)!} x^{2n +1} }
...
for n in range(5):
    y = spherical_jn(n, x)
    ax.plot(np.append(0, x), np.append(0, y), label=f'$j_{n}(x)$')
...
$$\begin{align*}
{\scriptsize (1 -x^2)}&{\scriptsize y'' -2xy' +n(n +1)y = }\\ &{\tiny [n(n+1)c_0 +2c_2] +[(n -1)(n +2)c_1 +6c_3]x }\\
     &{\tiny +\sum_{j=2}^\infty [(j +2)(j +1) c_{j +2} +(n -j)(n +j +1)c_j] x^j = 0}\\ 
 &\Downarrow \\
 {\tiny c_2} & {\tiny = -\frac{n(n+1)}{2} c_0 }\\ 
 {\tiny c_3 }& {\tiny = -\frac{(n -1)(n +2)}{6} c_1 }\\
 {\tiny c_{j +2} } & {\tiny =-\frac{(n -j)(n +j +1)}{(j +2)(j +1)} c_j, \;\; j=2,3,4,\cdots }
\end{align*}$$
